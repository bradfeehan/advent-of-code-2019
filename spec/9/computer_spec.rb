...
require File.join(ROOT, '9', 'computer')

RSpec.describe Intcode9::Computer do
  let(:computer) do
    described_class.new(
      initial_memory,
      stdout: stdout,
  ...
  context 'with day 9 examples' do
    ...
    context 'with example 1' do
      ...
      it { is_expected.to eq initial_memory.map(&:to_s).join("\n") }
    end

    context 'with example 2' do
      let(:initial_memory) { [1102, 34_915_192, 34_915_192, 7, 4, 7, 99, 0] }

      its(:length) { is_expected.to eq 16 }
      it { is_expected.to eq '1219070632396864' }

      it 'is a number' do
        expect { Integer(output) }.to_not raise_error
      end
    end

    context 'with example 3' do
      let(:initial_memory) { [104, 1_125_899_906_842_624, 99] }
      it { is_expected.to eq '1125899906842624' }
    end
  end
